...
RUN wget -qO- https://github.com/git/git/archive/v2.8.2.tar.gz | tar xvz

RUN cd git-2.8.2 && autoconf && ./configure --without-tcltk && make && make install && ln -s /usr/local/bin/git /usr/bin/git
